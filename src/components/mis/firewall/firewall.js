...
export default class Firewall extends Component{
  ...
  componentDidMount() {
    ...
    const param = `method=GetFireWallPortApplicationDetail&personId=${user.personId}&ApplicationID=${match.params.data}&encryptedstr=${md}`
  ...
  itemDomFunc = (type) => {
    ...
    const fireDate = data['FIREWALLPORT_DATA']
    ...
    if (type === 'base') {
      ...
      return Object.keys(detail).map((i, idx) => {
        return <Item
          key={idx}
          lt={detail[i]}
          rt={i !== 'REMARKS' ? fireDate[i] : data[i]}
        />
      })
    } else if (type === 'remark') {
